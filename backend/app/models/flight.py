...
from sqlalchemy import String, Text, DateTime, Date, Float, Integer, Boolean, ForeignKey, Index, func, text
...
class Flight(Base):
    ...
    review_status: Mapped[str] = mapped_column(String(20), default="needs_review")  # needs_review, reviewed
    pilot_confirmed: Mapped[bool] = mapped_column(Boolean, default=True)
    # Per-flight override of the pilot-level rule, for a one-off that is not
    # the unit's own activity (a vendor demo flown by one of our people, a
    # test flight). A flight counts only when both it and its pilot do.
    counts_toward_totals: Mapped[bool] = mapped_column(Boolean, default=True,
