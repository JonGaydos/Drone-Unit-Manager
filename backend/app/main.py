...
# Route application logs to stderr alongside uvicorn's HTTP access logs so
# operators reading `docker logs` see startup banners, audit traces, sync
# errors, and the fresh-install backup-import token. Must run BEFORE other
# modules import and bind their loggers. basicConfig is a no-op if the root
# logger already has handlers, so calling it here is safe regardless of
# uvicorn's own setup order.
logging.basicConfig(
    level=logging.INFO,
...
from app.routers import (
    ...
    components, geofences, adsb, notifications, calendar, geocode,
)
from app.routers import vehicle_registrations, backup, search
from app.routers import operating_authorities
...
async def lifespan(app: FastAPI):
    """Application lifespan handler: migrate schema, seed data, start scheduler."""
    # Main DB schema via Alembic (migrate -> seed order matters)
    ...
    seed_defaults()
    # Normalize dash-prefixed battery serials (Skydio API artifact) and fold
    # short-form duplicates (e.g. "-231117-1-00061" vs the full serial) into
    # their canonical records; idempotent, so safe every startup.
    from app.services.battery_dedupe import merge_duplicate_batteries
    _dedupe_db = SessionLocal()
    try:
        merge_duplicate_batteries(_dedupe_db)
    except Exception:
        logging.getLogger(__name__).exception("Battery dedupe failed; continuing startup")
    finally:
        _dedupe_db.close()
    # Generate/load the fresh-install backup-import token; logged for operators
    # to read out of container logs.
    from app.routers.backup import init_install_token
    init_install_token()
    start_scheduler()
    yield
    stop_scheduler()
...
# and silently disables CORS; fail loudly at startup rather than ship a footgun.
if "*" in settings.CORS_ORIGINS:
    raise RuntimeError(
        "CORS_ORIGINS may not contain '*' while allow_credentials=True. "
...
app.include_router(operating_authorities.router)
