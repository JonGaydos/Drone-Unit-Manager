...
def _fmt_change(old, new) -> dict:
    """Format an old/new value pair for the audit ``changes`` dict.

    Stringifies non-None values and preserves None, matching the shape the
    audit log renderer expects. Shared by ``compute_changes`` here and by the
    field-level recorder in the auth router.
    ...
    Returns:
        Dict ``{"old": str|None, "new": str|None}``.
    """
    return {
...
def compute_changes(old_obj, update_data: dict, fields: list[str]) -> dict:
    ...
    for field in fields:
        ...
        if old_val != new_val:
            changes[field] = _fmt_change(old_val, new_val)
    return changes
