...
def detect_format(content: str) -> str:
    ...
    Returns:
        Format identifier: "dji", "litchi", "airdata", or "unknown".
    """
...
def parse_csv_log(content: str, format_type: str) -> dict:
    ...
    alt_col = find(["altitude(m)", "altitude [m]", "altitude_m", "height"])
    ...
    for row in reader:
        ...
        # Convert mph to m/s if airdata format
        if speed is not None and format_type == "airdata" and speed_col and "mph" in speed_col.lower():
            speed = speed * 0.44704
