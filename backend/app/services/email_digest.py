...
def _build_expiring_registrations(db: Session, today: date, thirty_days: date) -> list[dict] | None:
    ...
    try:
        # Active fleet only — registrations on retired vehicles don't need renewal.
        exp_regs = db.query(VehicleRegistration).join(
            Vehicle, VehicleRegistration.vehicle_id == Vehicle.id
        ).filter(
