...
def complete_checklist(
    ...
    # all_passed must come from the TEMPLATE side: every required item must
    # have a matching response that is checked. Iterating from the responses
    # would let a missing required item silently pass.
    response_map = {r["label"]: r["checked"] for r in responses}
    required_items = [item for item in (template.items or []) if item.get("required", True)]
    all_passed = all(response_map.get(item.get("label"), False) for item in required_items)
    ...
    db.add(completion)
    db.flush()
    log_action(db, user.id, user.display_name, "create", "checklist_completion", completion.id,
               template.name,
               details=f"Completed checklist '{template.name}' (all_passed={all_passed})")
    db.commit()
