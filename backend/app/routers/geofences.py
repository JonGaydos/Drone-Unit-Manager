...
def _point_in_polygon(lat, lon, polygon):
    """Ray-casting algorithm for point-in-polygon. polygon is a list of
    (lat, lon) tuples. Casts a horizontal ray (varying lon) at the test
    latitude and counts edge crossings."""
    n = len(polygon)
    inside = False
    j = n - 1
    for i in range(n):
        ...
        # Edge crosses the test latitude (one vertex above, one below)
        if (lat_i > lat) != (lat_j > lat) and lat_j != lat_i:
            # x-coordinate (lon) where the edge intersects the horizontal ray
