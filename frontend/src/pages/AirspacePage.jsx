...
export default function AirspacePage() {
  ...
  // Start with no active search point; the map centers on the org default
  // location (Settings) on load, and the user clicks to choose a search point.
  ...
  const applyDefaultLocation = useCallback((settings) => {
    const { lat, lon } = resolveOrgLocation(settings)
    setMapCenter([lat, lon])
  }, [])

  // Always center on the org default location (Settings) on load.
  useEffect(() => {
    ...
    <div className="space-y-4">
      ...
      <div className="bg-card border border-border rounded-xl p-4">
        <div className="flex flex-wrap items-center gap-4">
          ...
          {/* Recent location chip — defers to org default, only a convenience */}
          {saved.lat && saved.lon && (selectedPoint?.[0] !== saved.lat || selectedPoint?.[1] !== saved.lon) && (
            <button
              type="button"
              onClick={() => handleMapClick(saved.lat, saved.lon)}
              className="flex items-center gap-1.5 px-2.5 py-1 bg-secondary border border-border rounded-lg text-xs text-muted-foreground hover:text-foreground hover:bg-muted/50"
              title="Jump to your last selected location"
            >
              <MapPin className="w-3.5 h-3.5" />
              Recent: {saved.lat.toFixed(2)}, {saved.lon.toFixed(2)}
            </button>
          )}
