...
import { ArrowLeft, Cpu, Clock, Edit, Users } from 'lucide-react'
...
export default function SensorDetailPage() {
  ...
  useEffect(() => {
    Promise.all([
      api.get(`/sensors/${id}`),
      api.get(`/sensors/${id}/stats`).catch(() => null),
      api.get(`/sensors/${id}/flights`).catch(() => []),
      api.get(`/maintenance?entity_type=sensor&entity_id=${id}`).catch(() => []),
    ]).then(([s, st, fl, m]) => {
      setSensor(s)
      setStats(st)
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const updated = await api.patch(`/sensors/${id}`, payload)
      setSensor(updated)
      setEditing(false)
    } catch (err) {
  ...
  if (!sensor) return <div className="text-center text-muted-foreground py-12">Sensor not found</div>

  const displayName = sensor.name || sensor.serial_number

  return (
    <div className="space-y-6">
      <Link to="/fleet?tab=sensors" className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground">
            ...
            {editing ? (
              ...
              <>
                ...
                <div className="flex flex-wrap gap-4 mt-2 text-sm text-muted-foreground">
                  {sensor.manufacturer && <span>{sensor.manufacturer}</span>}
                  {sensor.model && <span>{sensor.model}</span>}
                  {sensor.type && <span>Type: {sensor.type}</span>}
                  <span>S/N: {sensor.serial_number}</span>
                </div>
                <span className={`inline-flex mt-2 px-2 py-0.5 rounded-full text-xs font-medium ${STATUS_COLORS[sensor.status] || 'bg-zinc-500/15 text-zinc-400'}`}>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              {flights.length === 0 && <tr><td colSpan={6} className="px-4 py-8 text-center text-muted-foreground">No flights recorded for this sensor</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
      ...
      {sensor.notes && (
        <div className="bg-card border border-border rounded-xl p-4">
          ...
          <p className="text-sm text-muted-foreground whitespace-pre-wrap">{sensor.notes}</p>
        </div>
      )}

      {/* Documents */}
      <DocumentUpload entityType="sensor" entityId={id} />
    </div>
  )
}
