...
import { ArrowLeft, Battery, Clock, Edit, Zap, Activity, Loader2, Plus } from 'lucide-react'
...
export default function BatteryDetailPage() {
  ...
  const startEditing = () => {
    setEditForm({
      ...
      acquired_date: battery.acquired_date || '',
      decommissioned_date: battery.decommissioned_date || '',
      notes: battery.notes || '',
    })
    setEditing(true)
  }
    ...
    <div className="space-y-6">
            ...
            {editing ? (
              <div className="space-y-3">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div>
                    <label htmlFor="acquired-date" className="block text-xs font-medium text-muted-foreground mb-1">Acquired Date</label>
                    <input id="acquired-date" type="date" value={editForm.acquired_date} onChange={e => setEditForm({...editForm, acquired_date: e.target.value})}
                      className="w-full px-3 py-1.5 bg-secondary border border-border rounded-lg text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-ring" />
                  </div>
                  <div>
                    <label htmlFor="decommissioned-date" className="block text-xs font-medium text-muted-foreground mb-1">Decommissioned Date</label>
                    <input id="decommissioned-date" type="date" value={editForm.decommissioned_date} onChange={e => setEditForm({...editForm, decommissioned_date: e.target.value})}
                      className="w-full px-3 py-1.5 bg-secondary border border-border rounded-lg text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-ring" />
                  </div>
                </div>
                <div>
              ...
              <>
                ...
                <div className="flex flex-wrap gap-4 mt-2 text-sm text-muted-foreground">
                  ...
                  {battery.acquired_date && <span>Acquired: {battery.acquired_date}</span>}
                  {battery.decommissioned_date && <span>Decommissioned: {battery.decommissioned_date}</span>}
                </div>
      ...
      {showReadingForm && (
        <Modal
          ...
          title="Record Battery Health Reading"
          className="max-w-md"
        >
            <div className="space-y-3">
              <div>
