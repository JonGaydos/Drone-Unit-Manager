...
export default function PilotDetailPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      api.get(`/pilots/${id}/needs-attention`).catch(() => null),
    ]).then(([p, s, fData, c, cur, perf, na]) => {
      setPilot(p)
      setStats(s)
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Needs Attention — backend-aggregated lapsed currency and expired/expiring certs */}
      {(() => {
        ...
        return (
          <div className="bg-card border border-border rounded-xl overflow-hidden">
            <div className="px-4 py-3 border-b border-border flex items-center gap-2">
              ...
              {items.length > 0 && (
                <span className="ml-auto inline-flex items-center justify-center px-2 py-0.5 rounded-full text-xs font-medium bg-amber-500/15 text-amber-400">{items.length}</span>
              )}
            </div>
            <div className="p-4">
              {items.length === 0 ? (
