...
export default function VehicleDetailPage() {
  ...
  useEffect(() => {
    ...
    ]).then(([v, s, fData, bat, ctrl, sens, att, maint, regs, comp]) => {
      setVehicle(v)
      setStats(s)
  ...
  const startEditing = () => {
    setEditForm({
      ...
      provider_serial: vehicle.provider_serial || '',
            ...
            {editing ? (
              <div className="space-y-3">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div>
                    <label htmlFor="provider-serial" className="block text-xs font-medium text-muted-foreground mb-1">API Serial (what the provider calls this aircraft)</label>
                    <input id="provider-serial" type="text" value={editForm.provider_serial} onChange={e => setEditForm({...editForm, provider_serial: e.target.value})}
                      placeholder="e.g. SkydioX2-7646"
                      className="w-full px-3 py-1.5 bg-secondary border border-border rounded-lg text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-ring" />
                  </div>
                  <div>
              ...
              <>
                ...
                <div className="flex flex-wrap gap-4 mt-2 text-sm text-muted-foreground">
                  ...
                  {vehicle.provider_serial && <span>API: {vehicle.provider_serial}</span>}
