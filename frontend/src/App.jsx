      ...
      return (
        <div className="min-h-screen bg-bg flex items-center justify-center p-4">
          <div className="bg-card border border-border rounded-xl p-8 max-w-md text-center">
            ...
            <button onClick={() => globalThis.location.reload()} className="px-4 py-2 bg-primary text-primary-foreground rounded-lg">
              Reload Page
            </button>
          </div>
        </div>
...
const OtherEquipmentDetailPage = lazy(() => import('@/pages/OtherEquipmentDetailPage'))
...
const OperatingAuthorityPage = lazy(() => import('@/pages/OperatingAuthorityPage'))
...
/**
 * Route guard for admin-only pages. Mirrors ProtectedRoute but also requires
 * the authenticated user to be an admin; non-admins are redirected home.
 * UI polish only — the backend already enforces admin on these endpoints.
 * @param {Object} props
 * @param {React.ReactNode} props.children - Admin-only page content.
 */
    ...
    <Suspense fallback={<Spinner />}>
      <Routes>
        ...
        >
          ...
          <Route path="/fleet/other/:id" element={<OtherEquipmentDetailPage />} />
          ...
          <Route path="/operating-authority" element={<OperatingAuthorityPage />} />
